 ...
 * 
 * Every persistable class must have at least one field, getter or setter marked
 * with Persist(id=true).
 * 
...
public @interface PersistClass {
     ...
     * 
     * No rules are enforced, but generally -lower-case (or camelCase) names are
     * preferred. Also, singular names are preferred- so "player" instead of
     * "players".
     * 
     * Following these rules will ensure that auto-generated join table names
     * and ids won't look "strange".
     * 
     ...
     * Each schema gets its own connection and data store (database, file in
     * SqlLite).
     * 
     * Plugins are welcome to simply use their plugin name as the schema, though
     * that is not enforced.
     * 
     * Persistence puts some common objects in the "global" schema, it is not
     * necessarily recommended to use that in external plugins.
     * 
